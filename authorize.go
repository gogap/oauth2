...
func (s *Server) HandleAuthorizeRequest(w *Response, r *http.Request) *AuthorizeRequest {
	...
	if err != nil {
		w.SetErrorState(E_INVALID_REQUEST, err.Error(), "")
		w.InternalError = err
		return nil
	}
	...
	if ret.Client == nil {
		w.SetErrorState(E_UNAUTHORIZED_CLIENT, "client not exist", ret.State)
		return nil
	}
	if ret.Client.GetRedirectUri() == "" {
		w.SetErrorState(E_UNAUTHORIZED_CLIENT, "client redirect url not exist", ret.State)
		return nil
	}
	...
	if err = ValidateUriList(ret.Client.GetRedirectUri(), ret.RedirectUri, s.Config.RedirectUriSeparator); err != nil {
		w.SetErrorState(E_INVALID_REQUEST, err.Error(), ret.State)
		w.InternalError = err
		return nil
	}
